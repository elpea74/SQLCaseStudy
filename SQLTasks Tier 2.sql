...
SELECT DISTINCT f.name, CONCAT(firstname, ' ', surname) AS membername 
FROM Bookings b
...
WHERE starttime LIKE '2012-09-14%'
    AND CASE WHEN m.memid = 0 THEN f.guestcost * slots
...
WHERE starttime LIKE '2012-09-14%'
    AND CASE WHEN m.memid = 0 THEN f.guestcost * slots
        ELSE f.membercost * slots END > 30) AS sub
...
SELECT m1.firstname, m1.surname, 
 CASE WHEN m1.recommendedby <> 0 THEN CONCAT(m2.firstname, ' ', m2.surname) 
 ELSE 'N/A' END AS recommender FROM Members m1
LEFT JOIN Members m2 ON m1.recommendedby = m2.memid
ORDER BY m1.surname, m1.firstname

/* Q12: Find the facilities with their usage by member, but not guests */

SELECT f.name, COUNT(b.bookid) AS timesbooked 
FROM Facilities f
...
SELECT f.name, strftime('%m', b.starttime) AS month, COUNT(b.bookid) AS timesbooked 
FROM Facilities f
INNER JOIN Bookings b on b.facid = f.facid
WHERE b.memid <> 0
